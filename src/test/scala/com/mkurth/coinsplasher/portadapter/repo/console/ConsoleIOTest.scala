...
class ConsoleIOTest extends FlatSpec with Matchers {
  ...
  "ConsoleIO" should "write BuyOrder" in {
    consoleIo.orderToString(BuyOrder("BTC", 12, 0)) should be("buy BTC 12")
  }
  ...
  it should "write SellOrder" in {
    consoleIo.orderToString(SellOrder("XLM", 0.0012, 0)) should be("sell XLM 0.0012")
  }

  it should "read SellOrder" in {
    consoleIo.stringToOrder("sell OMG 0.00001") should be(Some(SellOrder("OMG", 0.00001, 0)))
  }
