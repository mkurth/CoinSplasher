...
case class Quota(price: BigDecimal, market_cap: BigDecimal, volume_24h: BigDecimal) {
  def withCurrency(currency: String) = QuotaWithCurrency(currency = currency, price = price, market_cap = market_cap, volume_24h = volume_24h)
}
...
object CoinInfoWithQuota {
  def apply(coinInfo: CoinInfo, quotes: Seq[QuotaWithCurrency]): CoinInfoWithQuota =
    new CoinInfoWithQuota(id = coinInfo.id, name = coinInfo.name, symbol = coinInfo.symbol, rank = coinInfo.rank, quotes = quotes)
}
...
class CoinMarketCap extends MarketRepo {
  ...
  def jsonToMarketCoins(blacklisted: Seq[CoinSymbol]): JsValue => Seq[MarketCoin] = { json =>
    {
      (json \ "data")
        .validate[JsObject]
        .get
        .fields
        .map({ case (key, value) => value.validate[CoinInfo].get })
  ...
  private def toCoinInfoWithQuota: CoinInfo => CoinInfoWithQuota = { coinInfo =>
    CoinInfoWithQuota(coinInfo,
                      coinInfo.quotes.fields
                        .map(
