...
case class CoinInfoWithQuota(id: Int, name: String, symbol: String, rank: Int, quotes: List[QuotaWithCurrency]) {
  def toCoin = Coin(
    symbol,
...
object CoinInfoWithQuota {
  def apply(coinInfo: CoinInfo, quotes: List[QuotaWithCurrency]): CoinInfoWithQuota =
...
class CoinMarketCap(implicit val ex: ExecutionContext) extends MarketRepo {
  ...
  override def loadMarketData(blacklisted: List[CoinSymbol] = List(), limitToCoins: Int = 20): Future[List[MarketCoin]] = {
    ...
    sttp.get(uri"$tickerURL")
      ...
      .map(_.body
        ...
        case Right(value: List[MarketCoin]) => value.take(limitToCoins)
      }
      )
  }

  def jsonToMarketCoins(blacklisted: List[CoinSymbol]): JsValue => List[MarketCoin] = {
    json => {
